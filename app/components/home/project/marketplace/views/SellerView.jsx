...
import { Package, TrendingUp, Eye, ShoppingCart, Clock, LayoutDashboard, Box, BarChart2, Truck, Lock } from 'lucide-react';
...
export default function SellerView() {
  ...
  return (
    <div className="flex w-full min-h-full">
      ...
      <aside className="hidden lg:flex w-64 bg-white border-r border-[#EBEAE5] flex-col shrink-0 sticky top-0 h-full">
        <div className="p-6 border-b border-[#EBEAE5]">
          ...
          <p className="text-xs font-bold text-[#5C6E63] mt-1">{currentUser.shopName}</p>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-3">
          ...
          {/* Menu Terkunci - Menggunakan Group Tooltip */}
          <div className="relative group">
            <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm text-[#9BA8A1] bg-white border border-transparent cursor-not-allowed hover:bg-slate-50 transition-colors">
              <Box size={18}/> Kelola Stok
              <Lock size={14} className="ml-auto text-[#C2BBAF]"/>
            </button>
            <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 w-48 bg-[#1C2C24] text-white text-[10px] px-3 py-2 rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 pointer-events-none shadow-xl">
              Fitur ini terkunci dalam mode live demo
            </div>
          </div>
      ...
      {/* MAIN SELLER CONTENT */}
      <div className="flex-1 max-w-6xl mx-auto px-4 md:px-8 py-8 space-y-8 pb-24">
        
        {/* Title Tampil di Mobile Karena Sidebar Disembunyikan */}
